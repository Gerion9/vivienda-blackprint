#!/usr/bin/env python3
"""
fact_avaluos_cdmx_audit.py
==========================

VERSIÓN DE AUDITORÍA de `fact_avaluos_cdmx.py`. La lógica funcional es
idéntica — mismo SQL, mismas operaciones, mismos resultados — pero antes y
después de cada operación importante se imprime a stdout un bloque de
auditoría con el estado del objeto (DataFrame, Series, ndarray, dict, etc.)
para que un humano pueda estudiar el flujo paso a paso y verificar que cada
sección de la plataforma ofrece datos fidedignos (sin "cálculos misteriosos").

Salida HTML por defecto: `eda_cdmx_audit.html` (no pisa al de producción).
Conviene redirigir stdout a un archivo para revisar los bloques con calma:

    export BLACKPRINT_DB_PASSWORD='...'
    python3 fact_avaluos_cdmx_audit.py > audit.log 2>&1
    python3 fact_avaluos_cdmx_audit.py --skip-cluster   # solo descriptiva
    python3 fact_avaluos_cdmx_audit.py --out reporte_audit.html

Cada bloque de auditoría se delimita con separadores `===` y muestra:
  · El propósito del paso (mismo nombre que figura en la consola).
  · Para DataFrames: shape, dtypes (primeras 12 cols), head(3), nulls top.
  · Para Series numéricas: min, p25, mediana, p75, max, mean, std.
  · Para Series categóricas: value_counts top 8.
  · Para ndarray: shape, dtype, primeras 8 entradas, min/max/mean/std.
  · Para dict/list: tipos, claves principales o primeros elementos.
...
# ---------------------------------------------------------------------------
# Utilitario de AUDITORÍA — imprime el estado de objetos antes/después de
# cada operación importante. No afecta la lógica del pipeline; sólo escribe a
# stdout. Pensado para que un humano lea la consola y verifique que cada paso
# está operando sobre los datos esperados.
# ---------------------------------------------------------------------------

_AUDIT_SEP = "=" * 88
_AUDIT_SUB = "-" * 88
_AUDIT_STEP_COUNTER = [0]   # mutable contador global de pasos
...
def _fmt_num(v) -> str:
    ...
    if isinstance(v, (float, np.floating)):
        ax = abs(float(v))
        if ax >= 1_000_000:    return f"{v:,.0f}"
        if ax >= 1:            return f"{v:,.2f}"
        return f"{v:.4f}"
    return str(v)
...
def _summarize_dataframe(df: "pd.DataFrame") -> list[str]:
    ...
    if len(cols) > 12:
        lines.append(f"  cols (primeras 12 de {len(cols)}): {cols[:12]}")
    else:
        lines.append(f"  cols ({len(cols)}): {cols}")
    lines.append("  dtypes (primeras 12):")
    for c, t in list(df.dtypes.items())[:12]:
        lines.append(f"    {c}: {t}")
    # nulos
    try:
        nulls = df.isna().sum()
        top_nulls = nulls[nulls > 0].sort_values(ascending=False).head(8)
        if len(top_nulls):
            lines.append("  nulls (top 8 cols con NaN):")
            n_total = max(len(df), 1)
            for c, n in top_nulls.items():
                lines.append(f"    {c}: {int(n):,} ({100.0*n/n_total:.1f}%)")
        else:
            lines.append("  nulls: ninguno en las columnas observadas")
    except Exception as e:
        lines.append(f"  nulls: <no se pudo calcular: {e}>")
    # head
    try:
        ...
        lines.append("  head(3):")
        for ln in head_str.splitlines():
            lines.append(f"    {ln}")
    except Exception as e:
        lines.append(f"  head: <no se pudo imprimir: {e}>")
    return lines


def _summarize_series(s: "pd.Series") -> list[str]:
    lines = [f"  len={len(s):,}  dtype={s.dtype}  name={s.name!r}"]
    try:
        n_null = int(s.isna().sum())
        lines.append(f"  nulls={n_null:,} ({100.0*n_null/max(len(s),1):.1f}%)")
    except Exception:
        pass
    numeric = pd.to_numeric(s, errors='coerce').dropna() if not pd.api.types.is_datetime64_any_dtype(s) else pd.Series([], dtype=float)
    if len(numeric):
        lines.append("  numérica · descriptivos:")
        lines.append(
            f"    min={_fmt_num(numeric.min())}  p25={_fmt_num(numeric.quantile(0.25))}  "
            f"med={_fmt_num(numeric.median())}  p75={_fmt_num(numeric.quantile(0.75))}  "
            f"max={_fmt_num(numeric.max())}  mean={_fmt_num(numeric.mean())}  "
            f"std={_fmt_num(numeric.std())}  n={len(numeric):,}"
        )
    else:
        try:
            vc = s.value_counts(dropna=False).head(8)
            lines.append("  value_counts top 8:")
            for v, c in vc.items():
                lines.append(f"    {v!r}: {int(c):,}")
        except Exception as e:
            lines.append(f"  value_counts: <no se pudo: {e}>")
    return lines


def _summarize_ndarray(arr: "np.ndarray") -> list[str]:
    ...
    try:
        flat = np.ravel(arr)
        sample = flat[:8].tolist() if flat.size <= 8 else flat[:8].tolist()
        lines.append(f"  primeras 8 entradas: {sample}")
        if np.issubdtype(arr.dtype, np.number):
            lines.append(
                f"  min={_fmt_num(float(np.nanmin(arr)))}  "
                f"max={_fmt_num(float(np.nanmax(arr)))}  "
                f"mean={_fmt_num(float(np.nanmean(arr)))}  "
                f"std={_fmt_num(float(np.nanstd(arr)))}"
            )
    except Exception as e:
        lines.append(f"  resumen numérico falló: {e}")
    return lines


def _summarize_mapping(d: dict) -> list[str]:
    lines = [f"  n_claves={len(d)}"]
    keys = list(d.keys())
    if not keys:
        lines.append("  (dict vacío)")
        return lines
    lines.append(f"  claves (primeras 12 de {len(keys)}): {keys[:12]}")
    # Muestra el valor de las primeras 3 claves
    for k in keys[:3]:
        v = d[k]
        if isinstance(v, pd.DataFrame):
            lines.append(f"  ['{k}'] -> DataFrame shape={v.shape}")
        elif isinstance(v, pd.Series):
            lines.append(f"  ['{k}'] -> Series len={len(v)} dtype={v.dtype}")
        elif isinstance(v, dict):
            sub_keys = list(v.keys())[:6]
            lines.append(f"  ['{k}'] -> dict (n={len(v)}) claves: {sub_keys}")
        elif isinstance(v, (list, tuple)):
            lines.append(f"  ['{k}'] -> {type(v).__name__} len={len(v)}")
        else:
            rep = repr(v)
            if len(rep) > 140: rep = rep[:140] + "…"
            lines.append(f"  ['{k}'] = {rep}")
    return lines


def _summarize_sequence(seq) -> list[str]:
    lines = [f"  len={len(seq)}  tipo={type(seq).__name__}"]
    for i, item in enumerate(list(seq)[:5]):
        rep = repr(item)
        if len(rep) > 140: rep = rep[:140] + "…"
        lines.append(f"    [{i}]: {rep}")
    if len(seq) > 5:
        lines.append(f"    ... ({len(seq)-5} más)")
    return lines


def audit_log(stage: str, note: str | None = None, **objects: Any) -> None:
    """Imprime un bloque de auditoría legible:
      · separador y nombre del paso (numerado)
      · nota opcional describiendo la operación
      · para cada objeto pasado por kwargs, un resumen apropiado a su tipo
    No retorna nada — sólo escribe a stdout."""
    _AUDIT_STEP_COUNTER[0] += 1
    step_n = _AUDIT_STEP_COUNTER[0]
    print(f"\n{_AUDIT_SEP}", flush=True)
    print(f"== AUDIT · paso #{step_n:03d} · {stage}", flush=True)
    if note:
        print(f"   {note}", flush=True)
    print(_AUDIT_SEP, flush=True)
    if not objects:
        print("  (sin objetos a inspeccionar)", flush=True)
    for name, obj in objects.items():
        print(f"\n  [{name}]  type={type(obj).__name__}", flush=True)
        try:
            if isinstance(obj, pd.DataFrame):
                for ln in _summarize_dataframe(obj):
                    print(ln, flush=True)
            elif isinstance(obj, pd.Series):
                for ln in _summarize_series(obj):
                    print(ln, flush=True)
            elif isinstance(obj, np.ndarray):
                for ln in _summarize_ndarray(obj):
                    print(ln, flush=True)
            elif isinstance(obj, dict):
                for ln in _summarize_mapping(obj):
                    print(ln, flush=True)
            elif isinstance(obj, (list, tuple, set)):
                for ln in _summarize_sequence(list(obj) if isinstance(obj, set) else obj):
                    print(ln, flush=True)
            elif isinstance(obj, (int, float, str, bool)) or obj is None:
                print(f"  valor = {obj!r}", flush=True)
            else:
                rep = repr(obj)
                if len(rep) > 400: rep = rep[:400] + "…"
                print(f"  repr: {rep}", flush=True)
        except Exception as e:
            print(f"  ⚠ no se pudo resumir [{name}]: {e}", flush=True)
    print(f"{_AUDIT_SEP}\n", flush=True)
...
def load_data(conn) -> pd.DataFrame:
    ...
    audit_log("load_data · INICIO",
              note=f"Query: SELECT ... WHERE cve_ent='{CVE_ENT_CDMX}' AND proposito='1' "
                   "(adquisición). Esperamos ~189k filas de avalúos de CDMX.",
              query=READ_QUERY.strip()[:600] + (" …" if len(READ_QUERY.strip()) > 600 else ""))
    df = pd.read_sql(READ_QUERY, conn)
    print(f"     · {len(df):,} filas")
    audit_log("load_data · POST read_sql",
              note="DataFrame crudo recién leído del staging. Verifica que cve_ent "
                   "sea sólo '09' y proposito sólo '1', y que las cols clave (lat/lon, "
                   "valor_concluido, sup_construida, ano, recamaras, banos) estén pobladas.",
              df=df,
              cve_ent_uniques=df["cve_ent"].value_counts(dropna=False) if "cve_ent" in df.columns else pd.Series([], dtype=int),
              proposito_uniques=df["proposito"].value_counts(dropna=False) if "proposito" in df.columns else pd.Series([], dtype=int),
              ano_distribution=df["ano"].value_counts(dropna=False).sort_index() if "ano" in df.columns else pd.Series([], dtype=int))
    # Derivar trimestre desde fecha_avaluo (Q1..Q4); NaT → NaN.
    audit_log("load_data · derivar trimestre",
              note="Vamos a calcular `trimestre` = quarter(fecha_avaluo). Fechas NaT "
                   "deben quedar como NaN, no como 0. Esperamos 4 valores únicos (1..4) "
                   "más NaN.",
              fecha_avaluo_sample=df["fecha_avaluo"].head(5) if "fecha_avaluo" in df.columns else None)
    fecha = pd.to_datetime(df["fecha_avaluo"], errors="coerce")
    df["trimestre"] = fecha.dt.quarter.astype("Int64")
    audit_log("load_data · FIN",
              note="DataFrame final post-derivación de trimestre. La distribución debe "
                   "ser ~25% por trimestre (uniforme dentro de cada año observado).",
              df=df,
              trimestre_distribution=df["trimestre"].value_counts(dropna=False).sort_index())
    return df


def load_pois(conn) -> pd.DataFrame:
    print("  → leyendo POIs de CDMX (dim_pois_dataplor)…")
    audit_log("load_pois · INICIO",
              note="POIs de dim_pois_dataplor filtrados al bbox CDMX. Se usarán para "
                   "contar amenidades dentro de un buffer alrededor de cada vivienda.")
    pois = pd.read_sql(POI_QUERY, conn)
    audit_log("load_pois · POST read_sql (antes de filtro de bbox)",
              note="POIs raw. Vamos a filtrar a lat∈[19.0,19.7] y lon∈[-99.5,-98.85] "
                   "(bbox CDMX), y a dropear nulos en lat/lon/main_category.",
              pois=pois,
              main_category_top10=pois["main_category"].value_counts(dropna=False).head(10) if "main_category" in pois.columns else pd.Series([], dtype=int))
    ...
    audit_log("load_pois · FIN",
              note="POIs post-filtro de bbox. Las top categorías serán usadas para "
                   "generar features `poi_<categoría>` por vivienda (ver attach_pois).",
              pois=pois,
              main_category_top10=pois["main_category"].value_counts().head(10))
    return pois


def load_nse_muni(conn) -> pd.DataFrame:
    print("  → leyendo NSE agregado a alcaldía (dim_socioeconomic_level_ageb_locality)…")
    audit_log("load_nse_muni · INICIO",
              note=f"NSE = promedio ponderado de tiers por alcaldía. Pesos: {NSE_TIER_WEIGHTS}. "
                   f"Resultado esperado: 16 alcaldías de CDMX.")
    nse = pd.read_sql(NSE_QUERY, conn)
    audit_log("load_nse_muni · POST read_sql (antes de derivar nse_score)",
              note="Tabla cruda de conteos por tier (a/b, c+, c, c-, d+, d, e) por alcaldía. "
                   "A continuación calculamos nse_score = Σ(conteo_tier · peso_tier)/Σ(conteo_tier).",
              nse=nse)
    ...
    audit_log("load_nse_muni · FIN (nse_score + nse_label derivados)",
              note="nse_score es continuo (pesos NSE_TIER_WEIGHTS). nse_label es la etiqueta "
                   "discreta más cercana. Verifica que cada alcaldía tenga un nse_score finito "
                   "y un nse_label en {A/B, C+, C, C-, D+, D, E}.",
              nse=nse,
              nse_score_dist=nse["nse_score"],
              nse_label_counts=nse["nse_label"].value_counts())
    return nse


def load_muni_geometries(conn) -> pd.DataFrame:
    print("  → leyendo geometrías de alcaldías (dim_mexico_geometries)…")
    audit_log("load_muni_geometries · INICIO",
              note="Lee polígonos de las 16 alcaldías para overlay en el mapa territorial.")
    g = pd.read_sql(MUNI_GEOM_QUERY, conn)
    print(f"     · {len(g)} polígonos.")
    audit_log("load_muni_geometries · FIN",
              note="DataFrame de geometrías. La columna geometry_coords_json es JSON/list "
                   "nested; se convierte a GeoJSON / shapely en pasos posteriores.",
              g=g)
    return g
...
def attach_pois(df: pd.DataFrame, pois: pd.DataFrame,
    ...
    audit_log("attach_pois · INICIO",
              note=f"Para cada vivienda con (lat, lon) válidas, contamos POIs dentro "
                   f"de un buffer de {buffer_m} m por cada una de las top-{top_n} "
                   f"`main_category`. Resultado: nuevas columnas `poi_<categoría>` "
                   f"(conteo entero ≥ 0). Las viviendas sin lat/lon quedan con 0.",
              df_in_shape=str(df.shape),
              pois_in_shape=str(pois.shape),
              top_n=top_n, buffer_m=buffer_m)
    ...
    audit_log("attach_pois · top categorías elegidas + POIs filtrados",
              note="Las categorías top dictan los nuevos features. Estos POIs filtrados "
                   "se cargan en un cKDTree (en coords XY de proyección equirectangular).",
              top_cats=top_cats,
              pois_top=pois_top)
    ...
    if has_geo.any():
        ...
        for k, nbrs in enumerate(neighbors):
            if nbrs:
                ...
                counts[idx_geo[k]] = bc
    audit_log("attach_pois · matriz de conteos calculada (antes de adjuntar al df)",
              note="`counts` es int (n_viviendas × top_cats). Cada fila es el #POIs "
                   "de cada categoría dentro del buffer alrededor de esa vivienda. "
                   "Filas correspondientes a viviendas sin lat/lon quedan en 0.",
              counts=counts,
              n_viviendas_con_geo=int(has_geo.sum()),
              n_viviendas_sin_geo=int((~has_geo).sum()))
    ...
    audit_log("attach_pois · FIN",
              note="DataFrame con columnas `poi_*` añadidas. Verifica que las medianas "
                   "y máximos sean plausibles (muchas viviendas tendrán 0, las del "
                   "centro tendrán decenas).",
              df=df[poi_cols] if poi_cols else df,
              poi_cols=poi_cols)
    return df, poi_cols
...
def attach_nse(df: pd.DataFrame, nse: pd.DataFrame) -> pd.DataFrame:
    ...
    audit_log("attach_nse · INICIO",
              note="Merge LEFT por cve_mun (normalizado a 3 dígitos con zfill). Cada "
                   "vivienda recibe nse_score / nse_label / conteo por tier de SU alcaldía. "
                   "Viviendas sin match (cve_mun fuera de CDMX) quedan con NaN en NSE.",
              df_shape=str(df.shape),
              nse_shape=str(nse.shape),
              df_cve_mun_sample=df["cve_mun"].head(5) if "cve_mun" in df.columns else None,
              nse_cve_mun_sample=nse["cve_mun"].head(5) if "cve_mun" in nse.columns else None)
    ...
    audit_log("attach_nse · POST merge",
              note="DataFrame con NSE adjunto. nse_label NaN ⇒ viviendas con cve_mun "
                   "que no matchea ninguna alcaldía en la tabla de NSE.",
              df=df,
              nse_label_distribution=df["nse_label"].value_counts(dropna=False) if "nse_label" in df.columns else pd.Series([], dtype=int))
    return df
...
def winsorize_block(df: pd.DataFrame) -> pd.DataFrame:
    ...
    audit_log("winsorize_block · INICIO",
              note="Recorta cada columna numérica a [p1, p99] (winsorización). Esto NO "
                   "altera la mediana ni los rangos intercuartílicos, sólo achata las "
                   "colas extremas para evitar que los outliers contaminen los promedios "
                   "y los descriptores del clustering.",
              cols_a_winsorizar=cols,
              **{f"{c}_before": df[c] for c in cols if c in df.columns})
    for c in cols:
        if c in df.columns:
            df[c] = winsorize(df[c], 0.01, 0.99)
    audit_log("winsorize_block · FIN",
              note="Cada columna recortada. Compara los descriptivos antes/después: "
                   "min y max deben moverse hacia el centro; mediana debe mantenerse.",
              **{f"{c}_after": df[c] for c in cols if c in df.columns})
    return df
...
def describe(df: pd.DataFrame) -> DescriptiveStats:
    audit_log("describe · INICIO",
              note="Calcula KPIs descriptivos para alimentar el hero, los recuadros "
                   "del year-picker, la tabla §01 (Top alcaldías) y los histogramas "
                   "globales del modal KPI. NO altera el df.",
              df=df,
              valor_concluido=df["valor_concluido"] if "valor_concluido" in df.columns else None,
              m2_sv=df["m2_sv"] if "m2_sv" in df.columns else None,
              valor_terreno_m2=df["valor_terreno_m2"] if "valor_terreno_m2" in df.columns else None)
    ...
    audit_log("describe · by_clase (groupby clase)",
              note="Cada fila = clase de valuador (1..7). n es el #avalúos con esa clase. "
                   "Las clases mayores son segmentos más altos.",
              by_clase=by_clase)
...
def cluster_kmeans(df: pd.DataFrame,
    ...
    audit_log("cluster_kmeans · INICIO",
              note="K-Means con features estandarizadas (StandardScaler), "
                   "random_state=42, n_init=10. Solo se usan filas SIN NaN en TODAS las "
                   "features. La k se elige por el #clases con n≥200, cap [3,6].",
              df_shape=str(df.shape),
              feature_cols=feature_cols)
    sub = df[feature_cols].copy()
    sub = sub.dropna()
    audit_log("cluster_kmeans · subset sin NaN",
              note="Sólo viviendas con TODAS las features válidas entran a clustering. "
                   "El resto queda con cluster NaN (no se les asigna submercado).",
              sub=sub,
              filas_descartadas=int(len(df) - len(sub)))
    if len(sub) < 500:
        audit_log("cluster_kmeans · ABORT",
                  note="Demasiadas pocas filas para clusterizar. Retornamos None y el "
                       "pipeline cae en modo --skip-cluster implícito.")
        return None
    ...
    audit_log("cluster_kmeans · k elegido",
              note="k = max(3, min(6, #clases_con_n≥200)). Idea: el #submercados "
                   "naturales se parece al #clases observadas con masa suficiente.",
              k=k,
              clase_counts=clase_counts,
              n_classes_relevant=n_classes_relevant)

    scaler = StandardScaler()
    X = scaler.fit_transform(sub)
    audit_log("cluster_kmeans · StandardScaler aplicado",
              note="Cada feature centrada en 0 con std=1. Después de scale: las medias "
                   "deben ser ~0 y las std ~1.",
              X=X,
              X_means_per_feature=X.mean(axis=0),
              X_stds_per_feature=X.std(axis=0),
              scaler_mean=scaler.mean_,
              scaler_scale=scaler.scale_)
    km = KMeans(n_clusters=k, random_state=42, n_init=10)
    labels = km.fit_predict(X)
    audit_log("cluster_kmeans · POST fit_predict",
              note="`labels` = id de cluster por fila de `sub`. La inercia mide qué tan "
                   "compactos son los clusters (más bajo = mejor).",
              labels=labels,
              cluster_sizes=pd.Series(labels).value_counts().sort_index(),
              inertia=float(km.inertia_),
              cluster_centers_shape=str(km.cluster_centers_.shape))
    ...
    profile = (aligned.groupby("_cluster")
                      ...
                      .reset_index(drop=True))
    audit_log("cluster_kmeans · perfil mediano por cluster (antes de remapear IDs)",
              note="Mediana de cada feature por cluster. Estos números deciden el nombre "
                   "humano del cluster (size/tipo/age/segmento).",
              profile=profile)
    ...
    audit_log("cluster_kmeans · POST remapeo de IDs (0 = más barato → k-1 = más caro)",
              note="Verifica que la columna med_valor crezca monótonamente del cluster 0 "
                   "al cluster k-1. Si no, hay un bug en el remap.",
              profile=profile,
              remap=remap,
              muni_dominant=muni_dominant)
    ...
    audit_log("cluster_kmeans · nombres asignados",
              note="Nombres human-readable de cada cluster (size·tipo·age·segmento). "
                   "NSE y alcaldía se omiten por pedido del usuario.",
              names=names)
    ...
    audit_log("cluster_kmeans · FIN",
              note="ClusterResult listo. `labels_full` está alineado con el df original "
                   "(NaN para viviendas que no entraron al clustering por NaN en features).",
              labels_full=labels_full,
              labels_distribution=labels_full.value_counts(dropna=False).sort_index(),
              feature_means_shape=str(feature_means.shape))
...
def prepare_map_points(df: pd.DataFrame, cluster: "ClusterResult | None",
    ...
    audit_log("prepare_map_points · INICIO",
              note=f"Prepara la matriz de puntos que se inyecta al JS del explorador "
                   f"territorial (§05). Filtra a lat/lon válidos en bbox CDMX. Si pasa "
                   f"de max_points={max_points} se muestrea estratificado por cluster.",
              df_shape=str(df.shape))
    work = df.copy()
    ...
    audit_log("prepare_map_points · POST filtros (lat/lon + cluster)",
              note="Sólo viviendas con lat/lon en CDMX y cluster asignado pasan. "
                   "Estos son los puntos antes de muestreo opcional.",
              work_shape=str(work.shape),
    ...
    result = {"points": points, "munis": munis,
              "n_total_geo": n_eligible, "n_sampled": int(len(work))}
    audit_log("prepare_map_points · FIN",
              note="Cada punto es un array de 16 entradas (lat, lon, cluster, año, "
                   "m2_sv, valor_concluido, clase, muni_idx, trimestre, recamaras, "
                   "banos, sup_const, sup_terreno, valor_terreno_m2, edad_meses, "
                   "valor_fisico_construccion). El JS lee estos índices por posición.",
              n_points=len(points),
              n_munis=len(munis),
...
def compute_cluster_data(df: pd.DataFrame,
    ...
    audit_log("compute_cluster_data · INICIO",
              note="Por cada cluster, precomputa histogramas y stats (mediana, mean, "
                   "p25, p75, etc.) por atributo y por año. Alimenta los modales que se "
                   "abren al hacer click en una cluster card o en la leyenda del mapa.",
              df_shape=str(df.shape),
              cluster_present=(cluster is not None))
    if cluster is None:
        audit_log("compute_cluster_data · ABORT (cluster is None)")
        return {}
    work = df.copy()
    work["_cluster"] = cluster.labels
    work = work.dropna(subset=["_cluster"])
    ...
    audit_log("compute_cluster_data · datos alineados",
              note="Sólo viviendas con cluster asignado pasan al loop por cluster.",
              work_shape=str(work.shape),
              years_all=years_all)
    ...
    audit_log("compute_cluster_data · FIN",
              note="`out` es dict[cluster_id] → {name, n_total, stats, histograms, "
                   "nse_dist, muni_top, years}. Los stats por cluster deben ser "
                   "consistentes con el perfil del clustering.",
              out=out,
              cluster_sizes={cid: meta["n_total"] for cid, meta in out.items()})
    return out
...
def compute_alcaldia_data(df: pd.DataFrame,
    ...
    audit_log("compute_alcaldia_data · INICIO",
              note="Por alcaldía: geometry GeoJSON + histogramas + stats agregados. "
                   "Esto alimenta el overlay de polígonos + tooltips del mapa territorial "
                   "y los modales de comparación de municipios (§01).",
              df_shape=str(df.shape),
              muni_geoms_shape=str(muni_geoms.shape))
    work = df.copy()
    if cluster is not None:
        work["_cluster"] = cluster.labels
    ...
    result = {"geojson": geojson, "by_cve": out_meta}
    audit_log("compute_alcaldia_data · FIN",
              note="geojson tiene una Feature por alcaldía con geometría + props (n, "
                   "med_valor, med_m2_sv, med_terreno, dom_cluster, nse_dist). by_cve "
                   "indexa los histogramas por cve_mun para el modal de comparación.",
              n_features=len(features),
              n_alcaldias_con_data=len(out_meta),
              muni_resumen={cve: {"name": meta["name"], "n": meta["props"]["n"]}
                            for cve, meta in list(out_meta.items())[:6]})
    return result
...
def prepare_phys_attr_points(df: pd.DataFrame, max_points: int = 220_000) -> dict:
    ...
    audit_log("prepare_phys_attr_points · INICIO",
              note="Subset para §02 (mapa de atributos físicos). Cada punto = "
                   "[lat, lon, clase, recamaras, banos, sup_construida, sup_terreno, "
                   "edad_meses, año, valor_concluido]. Sin clustering — el JS calcula "
                   "mediana/promedio en buffer dinámico.",
              df_shape=str(df.shape),
              max_points=max_points)
    ...
    if len(work) > max_points:
        work = work.sample(n=max_points, random_state=42)
    audit_log("prepare_phys_attr_points · POST filtros + sample",
              note="`work` después de filtrar lat/lon válidos en bbox y muestrear.",
              work_shape=str(work.shape))
    ...
    result = {"points": points, "n_total": int(len(work))}
    audit_log("prepare_phys_attr_points · FIN",
              note="Array compacto listo para inyectar en el JS de §02. Verifica que "
                   "el primer y último punto tengan lat/lon plausibles dentro de CDMX.",
              n_points=len(points),
              first_point=points[0] if points else None,
              last_point=points[-1] if points else None)
    return result
...
def compute_absorcion_data(df: pd.DataFrame, top_n: int = 20) -> dict:
    ...
    audit_log("compute_absorcion_data · INICIO",
              note=f"Calcula los top-{top_n} desarrolladores (columna `constructor`) "
                   "por #id_avaluo único, y emite filas compactas para que el JS "
                   "reagrupe por año/trimestre + filtre por atributos físicos en §07.",
              df_shape=str(df.shape),
              top_n=top_n,
              constructor_present=("constructor" in df.columns))
    if "constructor" not in df.columns:
        audit_log("compute_absorcion_data · ABORT (no hay columna 'constructor')")
    ...
    audit_log("compute_absorcion_data · ranking de desarrolladores",
              note="Estos son los desarrolladores que aparecerán en el gráfico §07. "
                   "Si el #1 te resulta raro, revisa que `constructor` esté bien capturado "
                   "en stg_avaluos (sin variantes con/sin acentos, mayúsculas, etc.).",
              top_devs=top_devs,
              counts_top20=counts.head(20))
    ...
    audit_log("compute_absorcion_data · FIN",
              note="Cada `row` = [dev_idx, ano, trimestre, clase, recamaras, banos, "
                   "sup_construida, sup_terreno, edad_meses, valor_concluido]. El JS "
                   "los reagrupa por (ano, trimestre) y dev_idx.",
              n_developers=len(top_devs),
              n_rows=len(rows),
              years=years,
...
def train_price_regression(df: pd.DataFrame,
    ...
    """
    audit_log("train_price_regression · INICIO",
              note="Regresión lineal en log(valor_concluido) sobre lat, lon, sup, "
                   "log(sup), recámaras, baños, edad, sqrt(edad), y dummies de cluster. "
                   "Sin train/test split. Verifica que las features y la dependiente "
                   "estén bien distribuidas antes de hacer fit.",
              df_shape=str(df.shape),
              cluster_present=(cluster is not None))
    work = df.copy()
    if cluster is not None:
    ...
    if cluster is not None:
        ...
        work["_cluster"] = work["_cluster"].astype(int)
    if len(work) < 1000:
        audit_log("train_price_regression · ABORT (n<1000)",
                  note="Demasiadas pocas filas con features completas. Retornamos None.",
                  n=int(len(work)))
        return None
    audit_log("train_price_regression · datos listos para regresión",
              note="`work` ya filtrado a filas sin NaN en las features requeridas. Este "
                   "es el universo sobre el cual se ajustan los coeficientes.",
              work_shape=str(work.shape),
              valor_concluido=work["valor_concluido"],
              sup_construida=work["sup_construida"],
              edad_meses=work["edad_meses"])
    ...
    audit_log("train_price_regression · matrices X / y construidas",
              note="X = matriz de features. y = log(valor_concluido). Cada feature debe "
                   "tener una distribución plausible (lat ~19.4, lon ~-99.1, sup ~80, etc.).",
    ...
    audit_log("train_price_regression · POST fit",
              note="Coeficientes y métricas finales. R² debe ser ~0.6-0.8. "
                   "Coeficientes deben tener signos plausibles: log_sup_const POSITIVO, "
                   "edad_meses cerca de 0 o negativo, etc. Si log_sup_const es negativo "
                   "es señal de bug en la matriz de features.",
              intercept=float(lr.intercept_),
              coefs=dict(zip(feature_names, [float(c) for c in lr.coef_])),
              r2=r2,
              sigma_log=sigma,
              n_train=int(n))
...
def timeseries_plusvalia(df: pd.DataFrame, cluster_labels: pd.Series | None
    ...
    """
    audit_log("timeseries_plusvalia · INICIO",
              note="Por cluster, mediana de m2_sv por año. La fórmula del índice de "
                   f"plusvalía es: idx_base = (med_año / med_año_base) - 1, donde el "
                   f"año base = primer año con n ≥ {N_MIN_TIMESERIES} (típicamente 2019). "
                   "NOMINAL: no se deflacta por INPC.",
              df_shape=str(df.shape),
              cluster_labels_present=(cluster_labels is not None),
              N_MIN_TIMESERIES=N_MIN_TIMESERIES,
    ...
    audit_log("timeseries_plusvalia · FIN",
              note="Por cluster, lista de records {ano, n, mediana_m2_sv, idx_base, "
                   "yoy, partial, insufficient}. Verifica que cada cluster tenga un "
                   "idx_base = 0% en su año base y que crezca/decrezca en los años "
                   "siguientes según el mercado.",
              clusters_in_out=list(out.keys()),
              records_per_cluster={cid: len(recs) for cid, recs in out.items()},
              cluster_0_sample=out.get(0, out.get("all"))[-3:] if out else None)
    return out
...
def main() -> None:
    ...
    ap.add_argument("--out", default="eda_cdmx_audit.html")
    ...
    audit_log("MAIN · INICIO DEL PIPELINE",
              note="Este es el script de AUDITORÍA. Cada paso imprimirá un bloque "
                   "antes/después con el estado de los objetos. Los pasos van a coincidir "
                   "1:1 con los del script de producción `fact_avaluos_cdmx.py`.",
              args=vars(args),
              constants={
                  "CVE_ENT_CDMX": CVE_ENT_CDMX,
                  "PHYSICAL_FEATURES": PHYSICAL_FEATURES,
                  "GEO_FEATURES": GEO_FEATURES,
                  "NSE_FEATURES": NSE_FEATURES,
    ...
    try:
        ...
        audit_log("main · phys_data preparado para §02",
                  note="Estos puntos se inyectan en el JS del mapa de Atributos Físicos.",
                  n_phys_points=len(phys_data["points"]))
        ...
        audit_log("MAIN · ANTES DE RENDERIZAR EL HTML",
                  note="Resumen final de objetos que entran al template. Si alguno se ve "
                       "vacío o con shape incorrecto, el HTML tendrá secciones sin datos.",
                  n_filas_df=int(len(df)),
                  has_cluster=(cluster is not None),
                  k_clusters=(cluster.k if cluster else 0),
                  n_clusters_with_data=len(cluster_data),
                  n_alcaldias_geom=len(alcaldia_data.get("by_cve", {})),
                  n_developers=len(absorcion_data.get("developers", [])),
        ...
        audit_log("MAIN · FIN DEL PIPELINE",
                  note=f"HTML de auditoría escrito en {args.out}. Total de pasos de "
                       f"auditoría emitidos: {_AUDIT_STEP_COUNTER[0]}. Si redirigiste "
                       f"stdout a un archivo, puedes abrirlo y leerlo en orden cronológico.")
    finally:
        conn.close()
